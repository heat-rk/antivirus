...
uint8_t ByteBuffer::getUInt8() const {
	uint8_t value = m_buf[m_rpos] + 128;
...
uint32_t ByteBuffer::getUInt32() const {
	uint32_t value =
		...
		((m_buf[m_rpos + 1] & 0xFF) << 16) +
		((m_buf[m_rpos + 2] & 0xFF) << 8) +
		((m_buf[m_rpos + 3] & 0xFF) << 0)) + 2147483648;
...
int64_t ByteBuffer::getInt64() const {
	int64_t value = 
		...
		((m_buf[m_rpos + 4] & 0xFF) << 24) +
		((m_buf[m_rpos + 5] & 0xFF) << 16) +
		((m_buf[m_rpos + 6] & 0xFF) << 8) +
		((m_buf[m_rpos + 7] & 0xFF) << 0);
